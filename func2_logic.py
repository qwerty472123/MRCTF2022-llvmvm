...
[QWORD* memory[0x21]] = key
[QWORD* memory[0x31]] = flag
memory[0xb5] = ckey [int arr]
memory[0xfd] = tkey [int arr]
...
for i in range(4):
    ...
    for j in range(0x64):
        for k in range(8):
            tmp += key[2 * k] ^ key[2 * k + 1]
            tmp &= 0xff
            tmp = SBox[tmp] + flag[8 * i + (k + 1) % 8]
            tmp &= 0xff
            tmp = (tmp << 1) | (tmp >> 7)
            tmp &= 0xff
            flag[8 * i + (k + 1) % 8] = tmp
...
for i in range(8):
    sumer = 0x0
    for j in range(0x20):
        t = (int)flag[4*i:4*i+4]
        sumer += weights[j] * ((t >> j) & 1)
        sumer %= mod
    if sumer != sums[i]:
        return 0
return 1