...
opcode_tbl = {
    ...
    vm_instr(
        ...
        instr_size=5,
        stackcost=-2,
        func=lambda operators: "memory[OP] = 4 * memory[{}] + memory[{}]".format(
                 hex(operators[1]), hex(operators[0]))),
    0xa:
...
while cursor < len(code):
    ...
    if opcode == 0x03:  # JUMP
        ...
        if opcode == 0x19:  # STACK operation
            operator_val = u32(operator)
...
def trace_execution(key):
    if key in lm:
        return lm[key]
    pc = 0x45
    while pc in range(0x45, 0x100) or pc in range(0x38b0, 0x4000):
        ...
        if target_val is None:
            pc = pc_1
            continue
        if code.startswith('if (([int* memory[0x88]] < 0x') and code.endswith('))'):
            cp = code[len('if (([int* memory[0x88]] < '):-2]
            cp = int(cp, 16)
            if key < cp:
                pc = target_val
            else:
...
for i, (cursor, code, target_val) in enumerate(lines):
    ...
    if nn is not None and nn[0] == 0x45:
        if code.startswith('memory[0x88] = (unsigned int*) '):
            ll = code[len('memory[0x88] = (unsigned int*) '):]
            ll = int(ll, 16)
...
for i, (cursor, code, target_val) in enumerate(lines):
    if code is None:
        continue
    if code.startswith('memory[0x88] = '):
        t1b3 = code[len('memory[0x88] = '):]
    if code.startswith('memory[0x90] = '):
        t1bb = code[len('memory[0x90] = '):]
